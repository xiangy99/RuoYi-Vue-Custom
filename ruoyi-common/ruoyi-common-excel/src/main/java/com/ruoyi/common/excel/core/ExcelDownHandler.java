...
public class ExcelDownHandler implements SheetWriteHandler {
        ...
        for (Map.Entry<Integer, FieldWrapper> entry : fieldCache.getSortedFieldMap().entrySet()) {
            ...
            if (field.isAnnotationPresent(ExcelDictFormat.class)) {
                ...
                if (StringUtils.isNotBlank(dictType)) {
                    ...
                    // 如果指定了确切的值，则直接解析确切的值
                    List<String> strList = this.splitList(converterExp, format.separator());
    ...
    private void dropDownLinkedOptions(DataValidationHelper helper, Workbook workbook, Sheet sheet,
        ...
        String linkedOptionsSheetName = String.format("%s_%d", LINKED_OPTIONS_SHEET_NAME,
        ...
        for (int columIndex = 0; columIndex < firstOptions.size(); columIndex++) {
            ...
            // 以第二行该列数据拼接引用位置
            String sonFunction = String.format("%s!$%s$2:$%s$%d", linkedOptionsSheetName, firstOptionsColumnName,
                    firstOptionsColumnName, secondOptions.size() + 1);
            // 设置名称管理器的引用位置
    ...
    private void dropDownWithSheet(DataValidationHelper helper, Workbook workbook, Sheet sheet, Integer celIndex,
        ...
        String function = String.format("%s!$%s$1:$%s$%d", OPTIONS_SHEET_NAME,
                getExcelColumnName(currentOptionsColumnIndex), getExcelColumnName(currentOptionsColumnIndex),
                value.size());
        // 设置名称管理器的引用位置
    ...
    private String getExcelColumnName(int columnIndex) {
        ...
        String columnPrefix = columnCircleCount == 0 ? StrUtil.EMPTY
     ...
     * @return 分割后的数据列表
     */
    private List<String> splitList(String str, String separator) {
        if (isBlank(str)) {
            return new ArrayList<>(0);
        }
        return StrUtil.split(str, separator).stream().filter(Objects::nonNull).map(Convert::toStr)
